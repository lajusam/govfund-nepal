...
const { fetchAllProjectsFromChain } = require('../services/solana');

// GET overall analytics — merges on-chain + MongoDB so ALL projects are counted
router.get('/', async (req, res) => {
    try {
        ...
        //    MongoDB-only projects are kept.
        ...
        res.json({
            overview: {
                ...
                activeProjects,
                completedProjects,
                utilizationRate: totalAllocated > 0 ? ((totalReleased / totalAllocated) * 100).toFixed(1) : 0,
            },
            provinceStats,
            sectorStats,
        });
